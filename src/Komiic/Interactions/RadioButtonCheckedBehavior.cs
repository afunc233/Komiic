...
/// <summary>
/// A behavior that listens for a <see cref="ToggleButton.IsChecked"/> property on its source and executes its actions when that event is fired.
/// </summary>
public class RadioButtonCheckedBehavior : Trigger
{
    ...
    protected override void OnAttached()
    {
        ...
        if (AssociatedObject is not Interactive interactive) return;
    ...
    private void RadioButtonOnIsCheckedChanged(object? sender, RoutedEventArgs e)
    {
        ...
        if (e.RoutedEvent != ToggleButton.IsCheckedChangedEvent) return;
